...
∂ΔT/∂t - c_T ∇²ΔT = λ_Tp ∂Δp/∂t + S_T
∂Δp/∂t - c_p ∇²Δp = λ_pT ∂ΔT/∂t + S_p
...
# Due to diffusion length L = √(κt) ~ 1000m after one year
# I implement a buffer zone of 1000m away from the wells
mesh_width = 2500.0    # x-direction [m]
mesh_height = 2000.0   # y-direction [m]
...
# Material properties
c_p = 1.0e-3        # Hydraulic diffusivity [m²/s]
c_T = 1.0e-4        # Thermal diffusivity [m²/s]
lambda_pT = 20.0e6  # Mechano-caloric coupling coefficient [Pa K^-1]
lambda_Tp = 0.01e-6 # Thermo-osmosis coupling coefficient [K Pa^-1]

# System properties
p_inj = 2e6          # Injection pressure [Pa]
p_ext = -1e6         # Extraction pressure [Pa]
T_inj = 20           # Injection temperature [C]
T_surf = 20           # Surface temperature [C]
T_bot = 200          # Bottom temperature at 6km depth [C]
well_depth = 5000.0  # Depth of the wells [m]

# Characteristic scales
Lc = 500.0                      # Distance between wells [m]
tc = Lc**2 / c_p              # Characteristic time scale of pressure diffusion
Deltapc = np.abs(p_inj - p_ext) # Pressure difference between wells [Pa]
DeltaTc = np.abs(T_bot - T_inj)    # Temperature difference between injection well and bottom [C]


# ------- DIMENSIONLESS PARAMETERS (SI units) -------------

epsilon = c_T / c_p                   # Relative diffusivity coefficient
chi = lambda_pT * DeltaTc / Deltapc        # Scaled thermal pressurization coefficient
gamma = lambda_Tp * Deltapc / DeltaTc    # Scaled poro-thermal coupling ratio

# Dimensionless source values
p_inj_scaled = p_inj / Deltapc
p_ext_scaled = p_ext / Deltapc
T_inj_scaled = (T_inj - T_bot) / DeltaTc
...
# M0_Tp = penalize(M_Tp, D=bc_dofs_T)
M0_Tp = M_Tp.copy()
M0_Tp[bc_dofs_T, :] = 0

# Pressure : no boundary
L0_p, M0_p = L_p, M_p #penalize(L_p, M_p, D=boundary_dofs_p)
M0_pT = M_pT #penalize(M_pT, D=boundary_dofs_p)
...
def dimensionalize_and_add_background(u_T_nd, u_p_nd, mesh):
    ...
    G = (T_bot - T_surf)/6000
    T_background = T_surf + T_surf - G * (y_coords_nodal - well_depth)
